    ...
    public IWebProxy? CurrentSystemProxy
    {
        get
        {
            switch (Configuration.Proxy)
            {
                ...
                default:
                    return GetOrCreateManualProxy(Configuration.Proxy);
            }
        }
    }

    private static readonly object _manualProxyLock = new();
    private static string? _manualProxyAddress;
    private static IWebProxy? _manualProxy;

    private static IWebProxy GetOrCreateManualProxy(string address)
    {
        lock (_manualProxyLock)
        {
            if (_manualProxy is null || _manualProxyAddress != address)
            {
                _manualProxyAddress = address;
                _manualProxy = new WebProxy(address);
            }
